...
$sql = sprintf("DELETE FROM `promocje` WHERE `promocje`.`id` = '%s'",
    mysqli_real_escape_string($link, $id));

if ($result = mysqli_query($link, $sql)) {
    $array = [true, "Pomyślnie usunięto promocje."];
} else {
    $array = [false, "Nie usunięto promocji - błąd:" . mysqli_error($link)];
}
