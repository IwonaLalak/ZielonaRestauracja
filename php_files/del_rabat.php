...
$sql = sprintf("UPDATE `users` SET `rabat` = 'Brak rabatu' WHERE `users`.`id` = '%s'",
    mysqli_real_escape_string($link, $id));


if ($result = mysqli_query($link, $sql)) {
    $array = [true, "Pomyślnie usunięto rabat."];
} else {
    $array = [false, "Nie usunięto rabatu - błąd:" . mysqli_error($link)];
}
